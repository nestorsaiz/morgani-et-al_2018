...
eb.cor <- function(dataset) {
        ...
        for(i in 1:length(embryos)) {
                ...
                coefs[i,1:2] <- summary(lm(log(CH2.Avg+0.0001) ~ Z + (TE_ICM=="TE"), 
                                           data=xxi))$coefficients[2,1:2]
                coefs[i,3:4] <- summary(lm(log(CH5.Avg+0.0001) ~ Z + (TE_ICM=="TE"), 
                                           data=xxi))$coefficients[2,1:2]
                coefs[i,5:6] <- summary(lm(log(CH3.Avg+0.0001) ~ Z + (TE_ICM=="TE"), 
                                           data=xxi))$coefficients[2,1:2]
        }
        ...
        dataset$CH2.ebLogCor <- rep(NA, nrow(dataset))
        dataset$CH5.ebLogCor <- rep(NA, nrow(dataset))
        dataset$CH3.ebLogCor <- rep(NA, nrow(dataset))
        for(i in 1:length(embryos)) {
                ii <- dataset$Embryo_ID == embryos[i]
                dataset$CH2.ebLogCor[ii] <- 
                        log(0.0001+dataset$CH2.Avg[ii]) - 
                        ebcoefs1[i] * dataset$Z[ii]
                dataset$CH5.ebLogCor[ii] <- 
                        log(0.0001+dataset$CH5.Avg[ii]) - 
                        ebcoefs2[i] * dataset$Z[ii]
                dataset$CH3.ebLogCor[ii] <- 
                        log(0.0001+dataset$CH3.Avg[ii]) - 
